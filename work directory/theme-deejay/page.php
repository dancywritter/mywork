					...
					if (count($px_meta_page) > 0) {
						...
						foreach ( $px_meta_page->children() as $px_node ) {
							if ( $px_node->getName() == "blog" ) {
								...
								$px_counter_node++;
  								if ( $px_node->px_gal_album <> "" and $px_node->px_gal_album <> "0" ) {
									get_template_part( 'page_gallery_albums', 'page' );
								}
							}else if ( $px_node->getName() == "album" ) {
								$px_counter_node++;
							...
							}elseif($px_node->getName() == "contact_info"){
							   $px_counter_node++;
							   get_template_part('page_contact_info','page');
