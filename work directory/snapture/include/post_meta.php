...
function cs_meta_post( $post ) {
	...
	if ( $post_xml <> "" ) {
		...
		$post_related = '';
		$cs_blog_img_type = '';
		$post_thumb_twitter_status = '';
		$post_thumb_facebook_status = '';
		$post_thumb_custom_status = '';
        ...
        <div class="option-sec row">
            <div class="opt-conts">
            	<ul class="form-elements noborder">
                    ...
                    <li class="to-field">
                        <select name="post_thumb_view" class="dropdown" onchange="javascript:new_toggle(this.value)">
							...
							<option <?php if($post_thumb_view=="Gallery")echo "selected";?> >Gallery</option>
                            <option <?php if($post_thumb_view=="Facebook")echo "selected";?> >Facebook</option>
                            <option <?php if($post_thumb_view=="Twitter")echo "selected";?> >Twitter</option>
                            <option <?php if($post_thumb_view=="Custom Status")echo "selected";?> value="Custom Status" >Custom Html/Shortcode</option>
                        </select>
                        <p></p>
                    </li>
						...
						<ul class="form-elements" id="post_thumb_map" style="display:<?php if($post_thumb_view=="Gallery")echo 'inline"';else echo 'none';?>" >
                           <li class="to-label"><label>Select Gallery</label></li>
                            <li class="to-field">
                                <select name="post_thumb_gallery" class="dropdown">
                                    ...
                                    <?php
                                        ...
                                        while ($wp_query->have_posts()) : $wp_query->the_post();
                                    	?>
                                        <option <?php if(get_the_ID()==$post_thumb_gallery)echo "selected";?> value="<?php the_ID()?>"><?php the_title()?></option>
                                    	<?php
                                        endwhile;
                                    ?>
                                </select>
                        ...
                        <ul class="form-elements" id="post_thumb_twitter" style="display:<?php if($post_thumb_view=="Twitter")echo 'inline"';else echo 'none';?>" >
                           <li class="to-label"><label>Twitter Status</label></li>
                            <li class="to-field">
                                <textarea name="post_thumb_twitter_status" rows="5" cols="20"><?php echo htmlentities($post_thumb_twitter_status);?></textarea>
                            </li>
                        </ul>
                        <ul class="form-elements" id="post_thumb_facebook" style="display:<?php if($post_thumb_view=="Facebook")echo 'inline"';else echo 'none';?>" >
                           <li class="to-label"><label>Facebok Status</label></li>
                           <li class="to-field"><textarea name="post_thumb_facebook_status" rows="5" cols="20"><?php echo htmlentities($post_thumb_facebook_status);?></textarea></li>
                        </ul>
                        <ul class="form-elements" id="post_thumb_custom_status" style="display:<?php if($post_thumb_view=="Custom Status")echo 'inline"';else echo 'none';?>" >
                           <li class="to-label"><label>Custom Html/Shortcode</label></li>
                            <li class="to-field">
                                <textarea name="post_thumb_custom_status" rows="5" cols="20"><?php echo htmlentities($post_thumb_custom_status);?></textarea>
                            </li>
                        </ul>
				...
				<ul class="form-elements noborder">
                    ...
                    <li class="to-field">
                        <select name="inside_post_thumb_view" class="dropdown" onchange="javascript:new_toggle_inside_post(this.value)">
                            ...
                            <option <?php if($inside_post_thumb_view=="Gallery")echo "selected";?> >Gallery</option>
                        </select>
                        <p></p>
                    </li>
                      ...
                      <ul class="form-elements" id="inside_post_thumb_map" style="display:<?php if($inside_post_thumb_view=="Gallery")echo 'inline"';else echo 'none';?>" >
                           <li class="to-label"><label>Select Gallery</label></li>
                            <li class="to-field">
                                <select name="inside_post_thumb_gallery" class="dropdown">
                                    <option value="0">-- Select Gallery --</option>
                                    <?php
                                        $query = array( 'posts_per_page' => '-1', 'post_type' => 'cs_gallery', 'orderby'=>'ID', 'post_status' => 'publish' );
                                        $wp_query = new WP_Query($query);
                                        while ($wp_query->have_posts()) : $wp_query->the_post();
                                    	?>
                                        <option <?php if(get_the_ID()==$inside_post_thumb_gallery)echo "selected";?> value="<?php the_ID()?>"><?php the_title()?></option>
                                    	<?php
                                        endwhile;
                                    ?>
                                </select>
				...
				<ul class="form-elements">
                    <li class="to-label"><label>Image Type</label></li>
                    <li class="to-field">
                        <select name="cs_blog_img_type" class="dropdown" >
                        	<option <?php if($cs_blog_img_type=="")echo "selected";?> value="">Box Size</option>
                            <option <?php if($cs_blog_img_type=="cs-big-square")echo "selected";?> value="cs-big-square">Big Square</option>
                            <option <?php if($cs_blog_img_type=="cs-horizental")echo "selected";?> value="cs-horizental">Horizontal</option>
                            <option <?php if($cs_blog_img_type=="cs-vertical")echo "selected";?> value="cs-vertical">Verticle</option>
                        </select>
                    </li>
                </ul>
                <ul class="form-elements">
		...
		if ( isset($_POST['post_meta_form']) and $_POST['post_meta_form'] == 1 ) {
			...
			function cs_meta_post_save( $post_id ) {
				if( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
					...
					if (empty($_POST["post_thumb_gallery"])){ $_POST["post_thumb_gallery"] = "";}
					...
					if (empty($_POST["inside_post_thumb_gallery"])){ $_POST["inside_post_thumb_gallery"] = "";}
					...
					if (empty($_POST["cs_blog_img_type"])){ $_POST["cs_blog_img_type"] = "";}
					if (empty($_POST['cs_blog_large_layout']))$_POST['cs_blog_large_layout'] = "";
					
					if (empty($_POST["post_thumb_twitter_status"])){ $_POST["post_thumb_twitter_status"] = "";}
					if (empty($_POST["post_thumb_facebook_status"])){ $_POST["post_thumb_facebook_status"] = "";}
					if (empty($_POST["post_thumb_custom_status"])){ $_POST["post_thumb_custom_status"] = "";}
					...
					if (empty($_POST["page_content_postion"])){ $_POST["page_content_postion"] = "";}
  						...
  						$sxe = new SimpleXMLElement("<cs_meta_post></cs_meta_post>");
							...
							$sxe->addChild('post_thumb_gallery', $_POST['post_thumb_gallery'] );
							$sxe->addChild('post_thumb_twitter_status', htmlentities($_POST['post_thumb_twitter_status']) );
							$sxe->addChild('post_thumb_facebook_status', htmlentities($_POST['post_thumb_facebook_status']) );
							$sxe->addChild('post_thumb_custom_status', htmlentities($_POST['post_thumb_custom_status']) );
							...
							$sxe->addChild('inside_post_thumb_gallery', $_POST['inside_post_thumb_gallery'] );
							...
							$sxe->addChild('cs_blog_img_type', $_POST['cs_blog_img_type'] );
