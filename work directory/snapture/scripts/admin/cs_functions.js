...
function cs_amimate(id){
 ...
 function closepopedup(id){
  ...
  $("#"+id).slideUp(800);
	...
	function gallery_view_element(id,counter){
		
		if (id=="gallery_full_page"){
			jQuery("#cs_gallery_view"+counter).show();
		} else {
			jQuery("#cs_gallery_view"+counter).hide();
		}
	}
	function new_toggle(id){
		if (id=="Single Image"){
			jQuery("#post_thumb_image, #post_thumb_audio, #post_thumb_video, #post_thumb_slider, #post_thumb_map, #post_thumb_facebook, #post_thumb_twitter, #post_thumb_custom_status").hide();
			...
			jQuery("#post_thumb_image, #post_thumb_audio, #post_thumb_video, #post_thumb_slider, #post_thumb_map, #post_thumb_facebook, #post_thumb_twitter, #post_thumb_custom_status").hide();
			jQuery("#post_thumb_facebook").show();
		}
		else if (id=="Twitter"){
			jQuery("#post_thumb_image, #post_thumb_audio, #post_thumb_video, #post_thumb_slider, #post_thumb_map, #post_thumb_facebook, #post_thumb_twitter, #post_thumb_custom_status").hide();
			jQuery("#post_thumb_twitter").show();
		}
		else if (id=="Custom Status"){
			jQuery("#post_thumb_image, #post_thumb_audio, #post_thumb_video, #post_thumb_slider, #post_thumb_map, #post_thumb_facebook, #post_thumb_twitter, #post_thumb_custom_status").hide();
			jQuery("#post_thumb_custom_status").show();
		}
		else jQuery("#post_thumb_image, #post_thumb_audio, #post_thumb_video, #post_thumb_slider, #post_thumb_map, #post_thumb_facebook, #post_thumb_twitter, #post_thumb_custom_status").hide();
	}
	...
	function cs_toggle_featured_post(id,counter){
  		if(id == "gird_view"){
			jQuery("#cs_featured_post_"+counter).show();

		}
		else {
			jQuery("#cs_featured_post_"+counter).hide();
		}
		
	}
	function cs_toggle_home_page_element(id,counter){
		
		
		for ( var i = 1; i <= 5; i++ ) {
			jQuery("#home_v"+i).hide();
			jQuery("#home_blog_detail_options").hide();
		}
		if (id=="home_blog_view"){
			jQuery("#home_v1").show();
			jQuery("#home_blog_detail_options").show();
		}
		else if (id=="home_portfolio_view"){
			jQuery("#home_v3").show();
		...
		var Data = [
			...
			{ "Class" : "column_75" , "title" : "75" , "element" : ["column"] },
			{ "Class" : "column_50" , "title" : "50" , "element" : ["column"] },
			{ "Class" : "column_25" , "title" : "25" , "element" : ["column"] },
		];
		...
		var counter_track = 0;
        function cs_add_track_to_list(home_url, theme_url){
			counter_track++;
...
// adding social network start
	...
	var counter_port_portfolio = 0;
		var count_port_portfolio_js = 0;
        function cs_add_port_portfolio(home_url, theme_url){
			counter_port_portfolio++;
			count_port_portfolio_js++;
			if ( count_port_portfolio_js > 0 ) jQuery("#port_portfolio_header").show("");
			var dataString = 'counter_port_portfolio=' + counter_port_portfolio + 
							'&port_other_info_title=' + jQuery("#port_other_info_title_dummy").val() +
							'&port_other_info_desc=' + jQuery("#port_other_info_desc").val() +
							'&port_other_info_icon=' + jQuery("#port_other_info_icon").val() +
							'&action=cs_add_port_portfolio';
			jQuery("#loading").html("<img src='"+theme_url+"/images/admin/ajax_loading.gif' />");
            jQuery.ajax({
                type:"POST",
                url: home_url+"/wp-admin/admin-ajax.php",
                ...
                success:function(response){
					...
					jQuery("#port_other_info_desc").val("");
					jQuery("#port_other_info_icon").val("");
					jQuery("#total_port_portfolio").append(response);
					jQuery("#loading").html("");
					closepopedup('add_port_other');
                }
            });
