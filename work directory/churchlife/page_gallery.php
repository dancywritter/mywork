    ...
    <?php
		if ($px_node->header_title <> '') { ?>
                <header class="px-heading-title">
                    <h2 class="px-section-title px-heading-color"><?php echo $px_node->header_title; ?></h2>
                </header>
        <?php  } ?>
            ...
            if ( $px_meta_gallery_options <> "" ) {
                for ( $i = $limit_start; $i < $limit_end; $i++ ) {
                    ...
                    $link_url = $px_xmlObject->gallery[$i]->link_url;
 					$image_url = px_attachment_image_src($path, 742, 378);
