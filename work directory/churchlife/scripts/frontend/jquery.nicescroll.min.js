	...
	var l = window.MutationObserver || window.WebKitMutationObserver || !1,
		...
		o = function() {
			...
			if (this.version = "3.5.4", this.name = "nicescroll", this.me = j, this.opt = {
				...
				for (var t in s.opt) "undefined" != typeof g[t] && (s.opt[t] = g[t]);
			...
			if (this.synched = function(a, b) {
				...
				} : function() {}, u.hastranslate3d && s.opt.enabletranslate3d ? (this.setScrollTop = function(a, b) {
			...
			this.getOffset = function() {
				...
				if (s.ishwscroll) s.rail.css({
					...
					var b = s.getOffset(),
						c = b.top,
						d = b.left,
						c = c + n(s.win, "border-top-width", !0);
					s.win.outerWidth(), s.win.innerWidth();
					var d = d + (s.rail.align ? s.win.outerWidth() - n(s.win, "border-right-width") - s.rail.width : n(s.win, "border-left-width")),
						e = s.opt.railoffset;
				...
				if (u.hasmstouch && s.css(s.ispage ? a("html") : s.win, {
					...
					var e = s.docscroll;
					s.ispage && (e = s.haswrapper ? s.win : s.doc), u.isie9mobile || s.css(e, {
						"overflow-y": "hidden"
					...
					if (s.ispage ? (h.css({
						...
						if (u.cantouch || s.istouchcapable || s.opt.touchbehavior || u.hasmstouch) {
							s.scrollmom = new q(s), s.ontouchstart = function(b) {
								...
								if (s.hasmoving = !1, !s.locked) {
									if (u.hasmstouch)
										for (var c = b.target ? b.target : !1; c;) {
									...
									if (!("clientX" in b) && "changedTouches" in b && (b.clientX = b.changedTouches[0].clientX, b.clientY = b.changedTouches[0].clientY), s.forcescreen && (d = b, b = {
										...
										var d = a(window).width(),
											...
											e = Math.max(0, g - e),
											d = Math.max(0, f - d);
										s.rail.drag.ck = !s.rail.scrollable && s.railh.scrollable ? e > 0 ? "v" : !1 : s.rail.scrollable && !s.railh.scrollable ? d > 0 ? "h" : !1 : !1, s.rail.drag.ck || (s.rail.drag.dl = "f")
							...
							s.ontouchmove = function(b, c) {
								...
								if (s.rail.drag && 2 == s.rail.drag.pt) {
									...
									if (s.hasmoving = !0, s.preventclick && !s.preventclick.click && (s.preventclick.click = s.preventclick.tg.onclick || !1, s.preventclick.tg.onclick = s.onpreventclick), b = a.extend({
										original: b
									}, b), "changedTouches" in b && (b.clientX = b.changedTouches[0].clientX, b.clientY = b.changedTouches[0].clientY), s.forcescreen) {
										var d = b;
										b = {
											original: b.original ? b.original : b
									...
									if (d = !1, s.rail.drag.dl) d = !0, "v" == s.rail.drag.dl ? j = s.rail.drag.sl : "h" == s.rail.drag.dl && (i = s.rail.drag.st);
									else {
										var e = Math.abs(e),
											h = Math.abs(h),
											k = s.opt.directionlockdeadzone;
										if ("v" == s.rail.drag.ck) {
						...
						if (s.onmousedown = function(a, b) {
							if (!s.rail.drag || 1 == s.rail.drag.pt) {
								...
								var c = s.getTarget(a);
								return !s.ispage && u.hasmousecapture && c.setCapture(), s.isiframe && !u.hasmousecapture && (s.saved.csspointerevents = s.doc.css("pointer-events"), s.css(s.doc, {
									"pointer-events": "none"
						...
						var t = s.win.attr("style"),
							h = parseFloat(s.win.css("width")) + 1;
				...
				if ("IFRAME" == this.doc[0].nodeName) {
					var v = function(b) {
						...
						}), "contentWindow" in this ? s.bind(this.contentWindow, "scroll", s.onscroll) : s.bind(c, "scroll", s.onscroll), s.opt.enablemousewheel && s.bind(c, "mousewheel", s.onmousewheel), s.opt.enablekeyboard && s.bind(c, u.isopera ? "keypress" : "keydown", s.onkeypress), (u.cantouch || s.opt.touchbehavior) && (s.bind(c, "mousedown", s.ontouchstart), s.bind(c, "mousemove", function(a) {
							s.ontouchmove(a, !0)
				...
				var d = s.getScrollTop(),
					e = s.getScrollLeft();
				return (0 > (s.newscrolly - d) * (b - d) || 0 > (s.newscrollx - e) * (a - e)) && s.cancelScroll(), 0 == s.opt.bouncescroll && (0 > b ? b = 0 : b > s.page.maxh && (b = s.page.maxh), 0 > a ? a = 0 : a > s.page.maxw && (a = s.page.maxw)), s.scrollrunning && a == s.newscrollx && b == s.newscrolly ? !1 : (s.newscrolly = b, s.newscrollx = a, s.newscrollspeed = c || !1, s.timer ? !1 : (s.timer = setTimeout(function() {
					var e, f, c = s.getScrollTop(),
						d = s.getScrollLeft();
					e = a - d, f = b - c, e = Math.round(Math.sqrt(Math.pow(e, 2) + Math.pow(f, 2))), e = s.newscrollspeed && 1 < s.newscrollspeed ? s.newscrollspeed : s.getTransitionSpeed(e), s.newscrollspeed && 1 >= s.newscrollspeed && (e *= s.newscrollspeed), s.prepareTransition(e, !0), s.timerscroll && s.timerscroll.tm && clearInterval(s.timerscroll.tm), e > 0 && (!s.scrollrunning && s.onscrollstart && s.onscrollstart.call(s, {
						type: "scrollstart",
						current: {
							x: d,
							y: c
				...
				var a = s.getScrollTop(),
					b = s.getScrollLeft();
				return s.setScrollTop(a), s.railh && s.setScrollLeft(b), s.noticeCursor(!1, a, b), s.cursorfreezed = !1, 0 > a ? a = 0 : a > s.page.maxh && (a = s.page.maxh), 0 > b ? b = 0 : b > s.page.maxw && (b = s.page.maxw), a != s.newscrolly || b != s.newscrollx ? s.doScrollPos(b, a, s.opt.snapbackspeed) : (s.onscrollend && s.scrollrunning && s.triggerScrollEnd(), s.scrollrunning = !1, void 0)
			}) : (this.doScrollLeft = function(a, b) {
				var c = s.scrollrunning ? s.newscrolly : s.getScrollTop();
				s.doScrollPos(a, c, b)
			}, this.doScrollTop = function(a, b) {
				...
				function d() {
					...
					var a = 0,
						b = sy = s.getScrollTop();
					if (s.dst.ay) {
						var b = s.bzscroll ? s.dst.py + s.bzscroll.getNow() * s.dst.ay : s.newscrolly,
							c = b - sy;
				...
				var c = 0,
					c = b ? Math.floor((s.scroll.y - a) * s.scrollratio.y) : (s.timer ? s.newscrolly : s.getScrollTop(!0)) - a;
				if (s.bouncescroll) {
					var d = Math.round(s.view.h / 2); - d > c ? c = -d : c > s.page.maxh + d && (c = s.page.maxh + d)
				}
				...
				var c = 0,
					c = b ? Math.floor((s.scroll.x - a) * s.scrollratio.x) : (s.timer ? s.newscrollx : s.getScrollLeft(!0)) - a;
				if (s.bouncescroll) {
					var d = Math.round(s.view.w / 2); - d > c ? c = -d : c > s.page.maxw + d && (c = s.page.maxw + d)
				}
				return s.cursorfreezed = !1, px = s.getScrollLeft(!0), 0 > c && 0 >= px || c > s.page.maxw && px >= s.page.maxw ? s.noticeCursor() : (s.doScrollLeft(c), void 0)
				...
				if (!s.zoomactive) {
					...
					return s.zoomrestore.style.width = s.win.css("width"), s.zoomrestore.style.height = s.win.css("height"), s.zoomrestore.padding = {
						...
						"z-index": f + 100,
						margin: "0px"
					}), c = s.win.css("backgroundColor"), ("" == c || /transparent|rgba\(0, 0, 0, 0\)|rgba\(0,0,0,0\)/.test(c)) && s.win.css("backgroundColor", "#fff"), s.rail.css({
						"z-index": f + 101
					}), s.zoom.css({
						"z-index": f + 102
					}), s.zoom.css("backgroundPosition", "0px -18px"), s.resizeZoom(), s.onzoomin && s.onzoomin.call(s), s.cancelEvent(b)
				}
