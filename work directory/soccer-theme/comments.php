...
?>
			...
			<div id="comments">
				<header>
					<h2 class="heading-color section-title uppercase">
						<?php echo comments_number(__('No Comments', 'Soccer Club'), __('1 Comment', 'Soccer Club'), __('% Comments', 'Soccer Club') );?>
                    </h2>
				</header>
                <ul>
				...
				<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // Are there comments to navigate through? ?>
					<div class="navigation">
						<div class="nav-previous"><?php previous_comments_link( __( '<span class="meta-nav">&larr;</span> Older Comments', 'Soccer Club') ); ?></div>
						<div class="nav-next"><?php next_comments_link( __( 'Newer Comments <span class="meta-nav">&rarr;</span>', 'Soccer Club') ); ?></div>
					</div> <!-- .navigation -->
		...
		<?php 
			global $post_id;
			$you_may_use = __( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', 'Soccer Club');
			$must_login = __( 'You must be <a href="%s">logged in</a> to post a comment.', 'Soccer Club');
			$logged_in_as = __('Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'Soccer Club');
			$required_fields_mark = ' ' . __('Required fields are marked %s', 'Soccer Club');
			...
			$defaults = array( 'fields' => apply_filters( 'comment_form_default_fields', 
				array(
					...
					'<span>'.( $req ? __( '(required)', 'Soccer Club') : '' ) .'</span>'.
					...
					'title_reply' => __( 'Leave a Comment', 'Soccer Club' ),
					'title_reply_to' => __( 'Leave a Reply to %s', 'Soccer Club' ),
					'cancel_reply_link' => __( 'Cancel reply', 'Soccer Club' ),
					'label_submit' => __( 'Submit', 'Soccer Club' ),); 
