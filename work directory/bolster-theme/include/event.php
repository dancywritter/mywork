...
// event start
	...
	function cs_event_register() {  
		$labels = array(
			'name' => 'Gigs',
			'add_new_item' => 'Add New Gigs',
			'edit_item' => 'Edit Gig',
			'new_item' => 'New Gig Item',
			'add_new' => 'Add New Gig',
			'view_item' => 'View Gig Item',
        ...
        register_post_type( 'gigs' , $args );  
			...
			// adding Manage Location start
				...
				$args = array(
					...
					'show_in_menu' => 'edit.php?post_type=gigs',
	...
	{
		  $labels = array(
			'name' => 'Gigs Categories',
			'search_items' => 'Search Gig Categories',
			'edit_item' => 'Edit Gig Category',
			'update_item' => 'Update Gig Category',
			'add_new_item' => 'Add New Category',
			'menu_name' => 'Gigs Categories',
		  ); 	
		  register_taxonomy('event-category',array('gigs'), array(
			'hierarchical' => true,
			'labels' => $labels,
			'show_ui' => true,
			'query_var' => true,
	...
	function cs_event_tag() {
		  ...
		  register_taxonomy('event-tag','gigs',array(
			'hierarchical' => false,
			'labels' => $labels,
			'show_ui' => true,
    ...
    {
        add_meta_box( 'event_meta', 'Event Options', 'cs_event_meta_data', 'gigs', 'normal', 'high' );
    }
	function cs_event_meta_data($post) {
		...
		if ( $cs_event_meta <> "" ) {
			$cs_xmlObject = new SimpleXMLElement($cs_event_meta);
				...
				$event_related = $cs_xmlObject->event_related;
				...
				$event_ticket_url = $cs_xmlObject->event_ticket_url;
    	...
    	<div class="page-wrap">
            <div class="option-sec" style="margin-bottom:0;">
                <div class="opt-conts">
                    ...
                    <ul class="form-elements">
                        <li class="to-label"><label>Ticket Price</label></li>
                        <li class="to-field">
                            <input type="text" id="event_ticket_price" name="event_ticket_price" value="<?php echo $event_ticket_price;?>" />
                        </li>
                    </ul>
                    <ul class="form-elements">
                        <li class="to-label"><label>Buy Ticket URL</label></li>
                        <li class="to-field">
                            <input type="text" id="event_ticket_url" name="event_ticket_url" value="<?php echo $event_ticket_url;?>" />
                        </li>
                    </ul>
                    <ul class="form-elements">
                    	<li class="to-label"><label>Select Artist</label></li>
                            <li class="to-field1" style=" height:auto; width:72%;">
                                <select name="inside_event_artists[]" multiple="multiple" style="height:auto;width:72%;">
                                    <option value="0">-- Select Artists --</option>
                                    <?php
                                        $query = array( 'posts_per_page' => '-1', 'post_type' => 'artists', 'orderby'=>'ID', 'post_status' => 'publish' );
                                        $wp_query = new WP_Query($query);
                                        while ($wp_query->have_posts()) : $wp_query->the_post();
                                    ?>
                                        <option value="<?php the_ID()?>" <?php if (in_array(get_the_ID(), $inside_event_artists)) { echo 'selected="selected"';}?>><?php the_title()?></option>
                                    <?php
                                        endwhile;
                                    ?>
                                </select>
                                <p>Press ctrl to add multiple artists</p>
                            </li>
                    </ul>
                    <ul class="form-elements">
                    ...
                    <ul class="form-elements noborder">
                        ...
                        <li class="to-field">
                        	...
                        	<select name="event_address" class="dropdown" onchange="javascript:inside_event_map_showhide(this)">
                            	<option value="0">-- Select Event Location --</option>
                                <?php
            ...
            <ul class="form-elements">
                    ...
                    <li class="to-field">
                    	<input type="text" id="event_rating" name="event_rating" value="<?php echo $event_rating;?>" />
                         <p>Enter Gigs Rating from 1 to 5</p>
                        </li>
                    <li class="to-desc">
