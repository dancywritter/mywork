...
(function (e) {
    ...
    e.fn.prettyPhoto = function (s) {
        ...
        function w(e, t) {
            resized = false;
            E(e, t);
            imageWidth = e, imageHeight2 = t;
            if ((p > v || h > d) && doresize && settings.allow_resize && !u) {
                resized = true, fitting = false;
                while (!fitting) {
                    if (p > v) {
                        imageWidth = v - 200;
                        imageHeight2 = t / e * imageWidth
                    } else if (h > d) {
                        imageHeight2 = d - 200;
                        imageWidth = e / t * imageHeight2
                    } else {
                        fitting = true
                    }
                    h = imageHeight2, p = imageWidth
                }
                if (p > v || h > d) {
                    w(p, h)
                }
                E(imageWidth, imageHeight2)
            }
            return {
                width: Math.floor(imageWidth),
                height: Math.floor(imageHeight2),
...
jQuery(document).ready(function(){
	$= jQuery;
    $("area[rel^='prettyPhoto']").live("click",function(event) {
    /* Act on the event */
    $.prettyPhoto.open($(this).attr("href"),"","");
    return false;
    });
    $("a[rel^='prettyPhoto']").live("click",function(event) {
    /* Act on the event */
    $.prettyPhoto.open($(this).attr("href"),"","");
    return false;
    });
