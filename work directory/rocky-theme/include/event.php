	...
	function cs_event_meta_data($post) {
		...
		if ( $cs_event_meta <> "" ) {
			$cs_xmlObject = new SimpleXMLElement($cs_event_meta);
				...
				$event_artists = $cs_xmlObject->artists_list;
		}
		else {
            ...
            <div class="option-sec" style="margin-bottom:0;">
                <div class="opt-conts">
                    ...
                    <ul class="form-elements">
                        ...
                        <li class="to-field">
                            <select name="event_rating" class="dropdown">
                                <?php for ( $i = 1; $i <= 5; $i++ ) {?>
                                    <option value="<?php echo $i?>" <?php if($event_rating == $i){ echo 'selected="selected"';}?>><?php echo $i?></option>
                                <?php }?>
                            </select>
                        </li>
                   </ul>
                   ...
                   <ul class="form-elements">
                    ...
                    <ul class="form-elements">
                        <li class="to-label"><label>Select Artists</label></li>
                        <li class="to-field">
                        	<?php
								$event_artists_array = array();
								if ( isset($event_artists->artists) ) {
									foreach ( $event_artists->artists as $val ) {
										$event_artists_array[] = (int)$val;
									}
								}
                            ?>
                        	<select multiple="multiple" name="event_artists[]" class="dropdown" style="height:120px !important;">
                                <?php
									query_posts( array('posts_per_page' => "-1", 'post_status' => 'publish', 'post_type' => 'artists') );
										while ( have_posts()) : the_post();
											$selected = "";
											if ( in_array( get_the_ID(), $event_artists_array )) {
												$selected = "selected";
											}
										?>
	                                        <option <?php echo $selected?> value="<?php the_ID()?>"><?php the_title()?></option>
                                        <?php
										endwhile;
                                ?>
                            </select>
