	...
	mejs.MediaElementPlayer.prototype = {
		...
		setPlayerSize: function(width,height) {
			...
			// detect 100% mode - use currentStyle for IE since css() doesn't return percentages
      		if (t.height.toString().indexOf('%') > 0 || t.$cs_node.css('max-width') === '100%' || (t.$cs_node[0].currentStyle && t.$cs_node[0].currentStyle.maxWidth === '100%')) {
		...
		remove: function() {
			...
			if (!t.isDynamic) {
				if (t.media.pluginType === 'native') {
					...
					//t.$cs_node.clone().insertBefore(t.container);
					//t.$cs_node.remove();
				}
				/*else*/ t.$cs_node.insertBefore(t.container)
			}
